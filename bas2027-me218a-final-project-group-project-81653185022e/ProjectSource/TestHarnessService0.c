...
ES_Event_t RunTestHarnessService0(ES_Event_t ThisEvent)
{
  ...
  switch (ThisEvent.EventType)
  {
    ...
    break;
    case ES_SHORT_TIMEOUT:   // lower the line & annhounce
    {
    ...
    case ES_NEW_KEY:   // announce
    {
      ...
      if ('R' == ThisEvent.EventParam)
      {
          ThisEvent.EventType = ES_REVERSE;
          PostServoService(ThisEvent);
      }
      
      if ('s' == ThisEvent.EventParam)
      {
          ThisEvent.EventType = ES_BUZZER;
          PostBuzzerService(ThisEvent);
      }
      
      if ('h' == ThisEvent.EventParam)
      {
          ThisEvent.EventType = ES_STOP_BUZZER;
          PostBuzzerService(ThisEvent);
      }
      
      if ('w' == ThisEvent.EventParam)
      {
          ThisEvent.EventType = ES_WELCOME;
      ...
      {
          ThisEvent.EventType = ES_LEDS_OFF;
