...
bool InitPotService(uint8_t Priority)
{
   ...
   *******************************************/
  DB_printf( "Move the potentiometer to post AD events to Service 1\n\r");
  ADC_ConfigAutoScan(BIT11HI);
  ADC_MultiRead(LastAD_Value);
  
  uint32_t potValue = LastAD_Value[0];
  DB_printf("Initial PotVal %d\n", potValue);
...
//      } break;
      case EV_AD_UPDATE: {
          ...
          uint16_t mappedVal = mapPotToServo(ThisEvent.EventParam, 0, 1023, 1250, 5000);
          ThisEvent.EventType = ES_MOVE;
          ThisEvent.EventParam = mappedVal;
          PostServoService(ThisEvent);
          DB_printf("\rEV_AD_UPDATE received %d\r\n", ThisEvent.EventParam);
